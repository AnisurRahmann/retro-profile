...
const BlogPost: React.FC = () => {
  ...
  const headings = post
    ? post.content
        ...
        .map(line => ({
          id: line.replace('## ', '').toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, ''),
          text: line.replace('## ', '')
        }))
    : [];
  ...
  const shareTwitter = () => {
    const text = encodeURIComponent(`${post.title} by @arshakil`);
  ...
  if (isShortPost) {
    return (
      <div className="min-h-screen">
        {/* Navigation */}
        <nav className="nav">
          <Link to="/" className="nav-logo">SHAKIL.</Link>
        ...
        <main className="insight-page">
          ...
          <article className="insight-article">
            <div className="insight-article-header">
              ...
              <span className="insight-article-date">{formatDate(post.date)}</span>
            </div>
            ...
            <div className="insight-article-tags">
              {post.tags.map(tag => (
                <span key={tag} className="insight-article-tag">#{tag}</span>
              ))}
            </div>
            ...
            <section className="more-insights">
              ...
              <div className="more-insights-grid">
                {relatedPosts.map(relatedPost => (
                  ...
                  >
                    <span className="more-insight-category">{relatedPost.category}</span>
                    <p className="more-insight-excerpt">{relatedPost.excerpt}</p>
                  </Link>
                ))}
              </div>
            </section>
          ...
          <article className="article-content" ref={contentRef}>
            ...
            <header className="article-header">
              <div className="article-header-meta">
                ...
                <span className="article-header-date">{formatDate(post.date)}</span>
                <span className="article-header-separator">·</span>
                <span className="article-header-readtime">{calculateReadTime(post.content)} min read</span>
              </div>
              ...
              <div className="article-header-tags">
                {post.tags.map(tag => (
                  <span key={tag} className="article-header-tag">{tag}</span>
                ))}
              </div>
            </header>
            ...
            <footer className="article-footer">
              ...
              <div className="author-card">
                ...
                <div className="author-info">
                  ...
                  <Link to="/#about" className="author-link">More about me →</Link>
                </div>
              </div>
            </footer>
                ...
                <nav className="toc-nav">
                  {headings.map(heading => (
                    <a
                      key={heading.id}
                      href={`#${heading.id}`}
                      className={`toc-link ${activeHeading === heading.id ? 'toc-link-active' : ''}`}
                    >
                      {heading.text}
                    </a>
                  ))}
