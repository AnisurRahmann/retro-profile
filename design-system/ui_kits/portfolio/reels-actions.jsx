/* global React */
// Instagram-style action rail (3 icons) + Socials sheet + Toast
...
// ---- Socials sheet (slides up) -------------------------------------
const SOCIALS = [
  { id:'github',   label:'GitHub',      handle:'@AnisurRahmann',          href:'https://github.com/AnisurRahmann',  icon:I.github },
  { id:'linkedin', label:'LinkedIn',    handle:'/in/shakil-ai',           href:'https://linkedin.com/in/shakil-ai', icon:I.linkedin },
  { id:'x',        label:'X',           handle:'@shakil_builds',          href:'https://x.com/shakil_builds',       icon:I.x },
  { id:'email',    label:'Email',       handle:'pshakilwizard@gmail.com', href:'mailto:pshakilwizard@gmail.com',    icon:I.mail },
];

function SocialsSheet({ open, onClose, onToast }) {
  useEffect(() => {
    if (!open) return;
  ...
  const copyLink = async () => {
    try { await navigator.clipboard.writeText(window.location.href); onToast('link copied'); }
        ...
        <div className="sheet-list">
          {SOCIALS.map(s => (
            <a key={s.id} href={s.href}
               target={s.href.startsWith('mailto')?'_self':'_blank'} rel="noopener"
               className="sheet-row">
              <span className="sheet-ic">{s.icon}</span>
          ...
          <button className="sheet-row sheet-row-btn" onClick={copyLink}>
            <span className="sheet-ic">{I.copy}</span>
            <span className="sheet-text">
...
function ActionBtn({ icon, onClick, label }) {
  return (
    <button className="act" onClick={onClick} aria-label={label} title={label}>
      {icon}
    </button>
  );
}
...
// ---- ActionRail (3 icons) -----------------------------------------
function ActionRail({ onToast, onOpenSocials }) {
  const actions = [
    {
      icon: I.download,
      label: 'Download resume',
      onClick: () => {
        const href = (window.__resources && window.__resources.resumePdf) || 'resume.pdf';
    ...
    },
    {
      icon: I.calendar,
      label: 'Book a call',
      onClick: () => window.open('https://cal.com/shakil', '_blank', 'noopener'),
    },
  ];
...
window.SocialsSheet = SocialsSheet;
window.Toast = Toast;
